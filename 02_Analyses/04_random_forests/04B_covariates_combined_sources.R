# random forests covariates combined sources
...
covar.cmb.rf.mods.file <- file.path(
  dirs$saved, 
  "randForest_covar_combined_list.rds"
)
covar.cmb.rf.mods <- redo.if(
  redo.var, 
  covar.cmb.rf.mods.file, 
  max.save.mb = max.save.size, 
  {
    ...
    lapply(cmb.methods, function(cmb.m) {
      ...
      train.elt <- list(cmb.m, "Taxon", "Train")
      train.data <- taxon.cmb.rf.data %>% 
        pluck(!!!train.elt) %>% 
        select(all_of(c(diagnosis.col.name, covars)))
      test.elt <- list(cmb.m, "Taxon", "Test")
