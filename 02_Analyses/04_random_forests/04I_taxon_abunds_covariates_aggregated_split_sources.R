...
taxon.agg.covar.rf.mods.file <- file.path(
  dirs$saved, 
  "randForest_taxon_aggregated_covar_list.rds"
)
taxon.agg.covar.rf.mods <- redo.if(
  redo.var, 
  taxon.agg.covar.rf.mods.file, 
  max.save.mb = max.save.size, 
  {
    repeat.cols <- c(unique(unlist(ctrl.vars)), diagnosis.col.name)
