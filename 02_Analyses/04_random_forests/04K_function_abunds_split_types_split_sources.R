# random forests function abundances split types split sources
...
func.rf.mods.file <- file.path(dirs$saved, "randForest_func_list.rds")
func.rf.mods <- redo.if(
  redo.var, 
  func.rf.mods.file, 
  max.save.mb = max.save.size, 
  {
    lapply(sources, function(source) {
      lapply(func.lvls, function(lvl) {
        ...
        remove.cols <- names(train.data)[names(train.data) %in% covars]
