# random forests taxonomic abundances split types combined sources
...
taxon.cmb.rf.mods.file <- file.path(
  dirs$saved,
  "randForest_taxon_combined_list.rds"
)
taxon.cmb.rf.mods <- redo.if(
  redo.var,
  taxon.cmb.rf.mods.file,
  max.save.mb = max.save.size,
  {
    lapply(cmb.methods, function(cmb.m) {
      lapply(taxon.lvls, function(lvl) {
        ...
        to.remove <- c(internalID.col.name, unique(unlist(covars)))
        remove.cols <- names(train.data)[names(train.data) %in% to.remove]
        train.data[, (remove.cols) := NULL]
        test.data <- taxon.cmb.rf.data %>% 
          pluck(cmb.m, lvl, "Test")
        test.data[, (remove.cols) := NULL]
